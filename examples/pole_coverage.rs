...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let drawable_plain: Drawable = core.drawable().into();
    ...
    let mut drawable_sm = drawable_plain.clone();
    drawable_sm.translate([shift, 0.0]);

    let mut core_fb = core.clone();
    core_fb.set_flux_barrier(Some(Box::new(barrier)))?;
    let mut drawable_fb: Drawable = core_fb.drawable().into();
    drawable_fb.translate([2.0 * shift, 0.0]);

    let fp = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(&format!("docs/img/pole_coverage.svg"));

    let bb = BoundingBox::new(-60.0e-3, 2.0 * shift + 60.0e-3, -60.0e-3, 60.0e-3);
    ...
    view.write_to_file(&fp, |cr| {
        ...
        drawable_plain.draw(cr)?;
        drawable_sm.draw(cr)?;

        // Surface magnets
        for mut m in core_fb.surface_magnets(&surface_magnets, true) {
            m.translate([1.0 * shift, 0.0]);
        ...
        // Interior magnets
        for mut m in core_fb.interior_magnets(true) {
            m.translate([2.0 * shift, 0.0]);
            m.into_drawable().draw(cr)?;
        }
